...
void reportHelp() {
  ...
  Serial.println("DELAY:<value>    - Set delay time in ms (unsigned long)");
...
void checkMotors(int pin) {
 // Read the duration of the LOW pulse (since HLFB pulls to GND when active)
  ...
  if (activeTime + inactiveTime > 0) {
    ...
    Serial.println(isStaticActive ? "0%" : "Inactive");
    if(!motorFailure) motorFailure = !isStaticActive;
  }
    ...
    if (c == '\n' || c == '\r') {
      if (idx > 0) {
        ...
        if (strncmp(cmd, "MCT:", 4) == 0) {
          ...
          unsigned long value = strtoul(cmd + 6, NULL, 10);
...
void loop() {
  ...
  if (running) {
    ...
    for (int pin = 9; pin <= 9 + mctValue-1; pin++) {
      checkMotors(pin);
    }
  ...
  delay(delayTime); // Wait for the specified delay time before the next reading
  delay(1);
  readCommands();
  ...
  if(motorFailure) {
    ...
    Serial.println("Motor failure detected!");
  } else {
